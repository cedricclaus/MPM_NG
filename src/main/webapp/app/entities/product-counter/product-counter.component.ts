...
currentAccount: any;
    ...
    loadAll() {
        this.productCounterService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/product-counter'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ProductCounter) {
        return item.id;
    }
    registerChangeInProductCounters() {
        this.eventSubscriber = this.eventManager.subscribe('productCounterListModification', (response) => this.loadAll());
    }
